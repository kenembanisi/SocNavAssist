...
from static_avoidance import StaticAvoidance
...
if __name__ == "__main__":
    ...
    try:
        ...
        ### Instantiate static avoidance
        dt = 0.2
        # ws_model = [[1,0,10.42]
        ws_model = [[1,0,1.17]]
        static_avo = StaticAvoidance(agent, ws_model, dt)
        ...
        while not rospy.is_shutdown():
            ...
            v_suitable_static = static_avo.compute_velocity(goal)

            # Update simulation:
            # v_opt, v_suitable = rvo_agent.compute_V_opt(goal, alpha=1, v_suitable_static)
            v_opt, v_suitable = rvo_agent.compute_V_opt(goal, alpha=1)

            # store states
            logger.store_data(v_opt[0], v_suitable_static)
            ...
            # rospy.loginfo(v_suitable_static)
