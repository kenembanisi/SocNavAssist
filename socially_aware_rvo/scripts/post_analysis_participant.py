...
def check_intrusion(agent_pos, pedestrian_pos, proxemics_level):
    ...
    agent_radius = 0.4
    if proxemics_level == "collision":
        space_radius = 0.25 + agent_radius
...
def main(args):
    ...
    # participant_ID = ['S03', 'S05', 'S06', 'S07', 'S08', 'S10', 'S11', \
    #                      'S13', 'S14', 'S15', 'S16', 'S17', 'S18', 'S19']
    # participant_ID = ['S15']
    ...
    if intrusions_analysis:
        #######################################  INTIMATE INTRUSIONS  ##########################################
        with open(summary_dir+'/summary_data_int_intrusions.csv', mode='w') as output:
                        ...
                        if trial_ID[4] == condition and trial_ID[3] == 'test':
                            ...
                            for i in range(num_pedestrians):
                                for j in range(n_frames):
                                    intimate_new[i].append(intimate[j][i])


                            total_intrusion_count = []
                            
                            for j in range(num_pedestrians):
                                # for i in range(9, n_frames-1):
                                i = 9
                                intrusion_count = 0
                                while i < n_frames-1:
                                    # if (intimate_new[j][i+1] - intimate_new[j][i]) == 1:
                                    #     intimate_int_.append(1)
                                    # else:
                                    #     intimate_int_.append(0)
                                    if (intimate_new[j][i+1] - intimate_new[j][i]) == 1:
                                        # intrusion = True
                                        intrusion_count += 1
                                        i += 10
                                        ...
                                        i += 1
                                total_intrusion_count.append(intrusion_count)
                                    

                            # num_intimate = sum(intimate_int)
                            # avg_num_intimate.append(num_intimate)
                            num_intimate = sum(total_intrusion_count)
                            avg_num_intimate.append(num_intimate)
        ...
        with open(summary_dir+'/summary_data_ttc.csv', mode='w') as output:
            ...
            for participant in participant_ID:
                ...
                for condition in conditions:
                    ...
                    for filename in os.listdir(log_directory):
                        ...
                        if trial_ID[4] == condition and trial_ID[3] == 'test':
                            ...
                            min_ttc_proximity = [ min([ calc_ttc([agent_x[j], agent_y[j]], [agent_v[j], agent_omega[j]], agent_theta[j], [pedestrian_x[i][j], pedestrian_y[i][j]], \
                                                        pedestrian_v[i][j], "collision", 3.6) for i in range(9) ]) for j in range(n_frames) ]
                            tmp = []
                            for i in range(n_frames):
                            ...
                            if len(tmp) > 0:
                                avg_min_ttc_proximity = statistics.mean(tmp)
                    ...
                    results_per_condition[condition] = sum(avg_avg_min_ttc_proximity)/len(avg_avg_min_ttc_proximity)
                    # results_per_condition['avg_avg_disagreement'] = sum(avg_avg_disagreement)/len(avg_avg_disagreement)

                    # reset the lists
                    avg_median_min_ttc.clear()
                    avg_avg_min_ttc_proximity.clear()
    ...
    if disagreement_analysis:
        #######################################  MEAN DISAGREEMENT  ##########################################
        with open(summary_dir+'/summary_data_disagreement.csv', mode='w') as output:
            ...
            for participant in participant_ID:
                ...
                for condition in conditions:
                    ...
                    # results_per_condition[condition] =  sum(avg_num_intimate)
                    # results_per_condition[condition] = sum(avg_median_min_ttc)/len(avg_median_min_ttc)
                    results_per_condition[condition] = sum(avg_avg_disagreement)/len(avg_avg_disagreement)
    ...
    if path_length:
        #######################################  PATH LENGTH  ##########################################
        with open(summary_dir+'/summary_data_path_length.csv', mode='w') as output:
            ...
            for participant in participant_ID:
                ...
                # comment:
                print('Path Length: Done with [ ' + participant + ' ]')
